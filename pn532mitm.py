...
if not remote or remote_type == 'READER':
        ...
        if not status or not emulator.data[:4] == 'D58D':
                print('Target Init failed:', emulator.errorcode, emulator.get_error_str(card.errorcode))
                if remote:
                        connection.close()
                os._exit(True)
        data= emulator.data
...
try:
        while 42:
                ...
                if not remote or remote_type == 'READER':
                        ...
                        if not status:
                                print('Target Get Data failed:', emulator.errorcode, emulator.get_error_str(card.errorcode))
