...
# Data Group Names
TAG_NAME = {
    EF_COM: "EF.COM Data Group Presence Map",
...
ISO19794_5_IMG_DTYPE = {"00": "JPEG", "01": "JPEG 2000"}

ISO19794_5_IMG_FTYPE = {"00": "JPG", "01": "JP2"}
...
def jmrtd_personalise(documentnumber, dob, expiry):
    ...
    data = (
        "621B04"
        + "%02x" % len(documentnumber)
        + passport.ToHex(documentnumber)
        + "04"
        + "%02x" % len(dob)
        + passport.ToHex(dob)
        + "04"
    ...
    if passport.errorcode == "6D00":
        ...
        data = (
            passport.ToHex(documentnumber)
            + passport.ToHex(calculate_check_digit(documentnumber))
            + passport.ToHex(dob)
            + passport.ToHex(calculate_check_digit(dob))
            + passport.ToHex(expiry)
...
if not FILES and BAC:
    ...
    while iterations:
        ...
        passport.MRPcompsoitecd = calculate_check_digit(
            passport.MRPnumber
...
if Jmrtd:
    ...
    jmrtd_personalise(
        ...
        mrz[FieldKeys[1] : FieldKeys[1] + 6],
        mrz[FieldKeys[2] : FieldKeys[2] + 6],
    )
    print(
        "JMRTD/vonJeek 3DES key set to: "
        + mrz[FieldKeys[0] : FieldKeys[0] + 9]
        + mrz[FieldKeys[1] : FieldKeys[1] + 6]
        + mrz[FieldKeys[2] : FieldKeys[2] + 6]
    )
